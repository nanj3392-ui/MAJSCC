...
parser = argparse.ArgumentParser(description='WITT')              #创建一个解析对象   argparse是用于解析命令行参数的标准模块
parser.add_argument('--training', default='training', action='store_true',            #向该对象中添加要关注的命令行参数和选项,可选参数需要在关键词前面加--
                    help='training or testing')
parser.add_argument('--trainset', type=str, default='CIFAR10',
                    choices=['CIFAR10', 'DIV2K'],
                    help='train dataset name')
parser.add_argument('--testset', type=str, default='CIFAR10',
                    choices=['kodak', 'CLIC21', 'CIFAR10', 'CIFAR100'],
...
parser.add_argument('--C', type=int, default=16,      #16,32,64,96,128,192
                    help='bottleneck dimension')
parser.add_argument('--multiple-snr', type=str, default='-3,0,3,6,9,12,15',      #信噪比均匀设置为1-13
                    help='random or fixed snr')
args = parser.parse_args()              #调用parse_args()方法来解析命令行参数，解析成功之后即可使用，让你的脚本在运行时以不同的方式进行配置，而不需要修改脚本本身。
...
class config():
    ...
    device = torch.device("cuda:0")        #修改为在cup上运行
    norm = False
    # logger
    print_step = 100
    filename = datetime.now().__str__()[:-7]
    # workdir = r'D:\paper\paper_coding\WITT-main\history\{}'.format(filename)    #原本
    workdir = 'D:\BaiduSyncdisk\paper\paper_coding\WITT A Wireless Image Transmission Transformer For Semantic Communications\history'
    log = workdir + '/Log_{}.log'    #log'.format(filename)    去掉之后的这部分
    ...
    if args.trainset == 'CIFAR10':
        save_model_freq = 5
        image_dims = (3, 32, 32)
        train_data_dir = r"D:\BaiduSyncdisk\paper\paper_coding\Adatasets\CIFAR-10"
        if args.testset == 'CIFAR10':
            test_data_dir = r"D:\BaiduSyncdisk\paper\paper_coding\Adatasets\CIFAR-10"
        elif args.testset == 'CIFAR100':
            test_data_dir = r"D:\BaiduSyncdisk\paper\paper_coding\Adatasets\CIFAR-100"
        # test_data_dir = r"D:\BaiduSyncdisk\paper\paper_coding\Adatasets\CIFAR-10"
        batch_size = 128
        downsample = 2
        encoder_kwargs = dict(
        ...
        decoder_kwargs = dict(
            img_size=(image_dims[1], image_dims[2]),   #图像尺寸  32*32
        ...
        train_data_dir = [r"D:\BaiduSyncdisk\paper\paper_coding\Adatasets\DIV2K_train_HR/"]
        if args.testset == 'kodak':
            test_data_dir = [r"D:\BaiduSyncdisk\paper\paper_coding\Adatasets\Kodak24"]
...
def load_weights(model_path):
    pretrained = torch.load(model_path)   #添加语句map_location ='cpu'   map_location = torch.device('cpu')
    net.load_state_dict(pretrained, strict=True)
    del pretrained   #del 可以用来删除字典、列表的元素。
...
if __name__ == '__main__':
    ...
    # model_path = r"D:\paper\paper_coding\WITT-main\WITT_model\awgn\CIFAR10\WITT_AWGN_CIFAR10_fixed_snr10_psnr_C24.model"
    # load_weights(model_path)
    net = net.cuda()   #  用.cpu()  替换  .cuda()
    ...
    if args.training:
        for epoch in range(steps_epoch, config.top_epoch):
            train_one_epoch(args)
            if (epoch + 1) % config.save_model_freq == 0:
                save_model(net, r"D:\BaiduSyncdisk\paper\paper_coding\WITT A Wireless Image Transmission Transformer For Semantic Communications\WITT_model\awgn\CIFAR10\WITT_AWGN_CR16_CIFAR10.pth")   #save_path=config.models + '/{}_EP{}.model'.format(config.filename, epoch + 1))
                test()
    else:
        test()
