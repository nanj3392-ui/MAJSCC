import torch.nn as nn
import numpy as np
import torch
import os
import time


class Channel(nn.Module):
    """
    Currently the channel model is either error free, erasure channel, 无损信道和擦除信道
    rayleigh channel or the AWGN channel.
    """

    def __init__(self, args, config):
        super(Channel, self).__init__()
        self.config = config
        self.chan_type = args.channel_type   #信道类型
        self.device = config.device          #加载到训练设备上
        self.h = torch.sqrt(torch.randn(1) ** 2           #生成一个二维空间中的随机单位向量，并对该向量进行归一化处理
                            + torch.randn(1) ** 2) / 1.414
        if config.logger:   #将信道类型和信噪比写入日志
            config.logger.info('【Channel】: Built {} channel, SNR {} dB.'.format(    #用于记录信息级别的日志消息
                args.channel_type, args.multiple_snr))

    def gaussian_noise_layer(self, input_layer, std, name=None):  #定义加性高斯噪声  零均值
        device = input_layer.get_device()   #获取输入张量的设备
        noise_real = torch.normal(mean=0.0, std=std, size=np.shape(input_layer), device=device)
        noise_imag = torch.normal(mean=0.0, std=std, size=np.shape(input_layer), device=device)
        noise = noise_real + 1j * noise_imag
        return input_layer + noise

    def rayleigh_noise_layer(self, input_layer, std, name=None): #定义瑞利衰弱噪声
        noise_real = torch.normal(mean=0.0, std=std, size=np.shape(input_layer))
        noise_imag = torch.normal(mean=0.0, std=std, size=np.shape(input_layer))
        noise = noise_real + 1j * noise_imag
        h = torch.sqrt(torch.normal(mean=0.0, std=1, size=np.shape(input_layer)) ** 2
                       + torch.normal(mean=0.0, std=1, size=np.shape(input_layer)) ** 2) / np.sqrt(2)   #按照某种方式缩放，引入一些噪声到模型的输入
        if self.config.CUDA:
            noise = noise.to(input_layer.get_device())
            h = h.to(input_layer.get_device())
        return input_layer * h + noise

    def complex_normalize(self, x, power):  #对输入的张量进行归一化
        pwr = torch.mean(x ** 2) * 2
        out = np.sqrt(power) * x / torch.sqrt(pwr)
        return out, pwr


    def complex_forward(self, channel_in, chan_param):
        if self.chan_type == 0 or self.chan_type == 'none':   #无信道类型
            return channel_in

        elif self.chan_type == 1 or self.chan_type == 'awgn':
            channel_tx = channel_in
            sigma = np.sqrt(1.0 / (2 * 10 ** (chan_param / 10)))    #通过信噪比的计算来推导噪声的标准差
            chan_output = self.gaussian_noise_layer(channel_tx,
                                                    std=sigma,
                                                    name="awgn_Cnoise")
            return chan_output

        elif self.chan_type == 2 or self.chan_type == 'rayleigh':
            channel_tx = channel_in
            sigma = np.sqrt(1.0 / (2 * 10 ** (chan_param / 10)))
            chan_output = self.rayleigh_noise_layer(channel_tx,
                                                    std=sigma,
                                                    name="rayl_Cnoise")
            return chan_output

    def forward(self, input, chan_param, avg_pwr=False):
        if avg_pwr:
            power = 1
            channel_tx = np.sqrt(power) * input / torch.sqrt(avg_pwr * 2)
        else:
            channel_tx, pwr = self.complex_normalize(input, power=1)
        input_shape = channel_tx.shape
        channel_in = channel_tx.reshape(-1)  #将张量扁平化为一维数组
        L = channel_in.shape[0]    #获取张量第一维度的大小给L
        channel_in = channel_in[:L // 2] + channel_in[L // 2:] * 1j    #乘以虚数单位 j，将后一半的元素转换为虚部，，切片操作。前一部分为实部，后一部分为虚部。
        channel_output = self.complex_forward(channel_in, chan_param)    #输入信道 和 信噪比
        channel_output = torch.cat([torch.real(channel_output), torch.imag(channel_output)])   #连接噪声的实部和虚部
        channel_output = channel_output.reshape(input_shape)    #正在将名为 channel_output 的张量重新塑形（reshape），使其具有由 input_shape 指定的形状
        if self.chan_type == 1 or self.chan_type == 'awgn':
            noise = (channel_output - channel_tx).detach()
            noise.requires_grad = False
            channel_tx = channel_tx + noise
            if avg_pwr:
                return channel_tx * torch.sqrt(avg_pwr * 2)
            else:
                return channel_tx * torch.sqrt(pwr)
        elif self.chan_type == 2 or self.chan_type == 'rayleigh':
            if avg_pwr:
                return channel_output * torch.sqrt(avg_pwr * 2)
            else:
                return channel_output * torch.sqrt(pwr)




    def noiseless_forward(self, channel_in):
        channel_tx = self.normalize(channel_in, power=1)
        return channel_tx


